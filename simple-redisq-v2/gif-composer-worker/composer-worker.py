...
def execute_composer(log, output):
    gif = output.get('output')
    if gif:
        os.system(f"./gif-composer.sh {output}")
        log.info('Done')
    else:
        log.info('No output given.')
...
def main():
    ...
    watch_queue(
        redis_conn, 
        QUEUE_NAME, 
        lambda task_descr: execute_composer(named_logging, task_descr))
