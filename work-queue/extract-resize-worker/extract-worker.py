...
# # def execute_extract(container, video):
# def execute_extract(video):
#     os.system(f"./extract-frames.sh {video}")
#     # os.system(f"docker build -f Dockerfile -t {container} .")
#     # os.system(f"docker run -v $(pwd):/app {container} {video}")
...
def main():
    ...
    watch_queue(
        redis_conn, 
        QUEUE_NAME, 
        lambda task_descr: execute_extract(named_logging, task_descr))
